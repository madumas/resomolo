...
- **Surlignage par mots :** L'enfant peut surligner le texte en 4 couleurs (bleu = données connues, orange = question, vert = contexte relationnel comme « de plus », « fois plus », « le reste », gris = pas utile — données non pertinentes). Stratégie universelle en orthopédagogie pour la compréhension d'énoncés. La 4e couleur (gris, Pas utile) soutient l'inhibition cognitive : l'enfant identifie explicitement les informations qu'il doit ignorer. **Interaction sans drag :** l'enfant choisit une pastille de couleur, puis clique sur les mots un par un. Un clic sur un mot adjacent à un mot déjà surligné de la même couleur étend la sélection. Un clic sur un mot non-adjacent crée un nouveau segment surligné. Un clic sur un mot déjà surligné le désélectionne. Zéro drag — compensation motrice complète. Chaque pastille de couleur affiche un label texte visible à l'intérieur (« Données », « Question », « Contexte », « Pas utile ») plutôt que de reposer uniquement sur les tooltips. Amélioration d'accessibilité : les tooltips HTML ne fonctionnent pas sur tablette, et un enfant daltonien ne distingue pas les couleurs sans texte.
...
- **Tout effacer :** Bouton qui vide les pièces mais garde l'énoncé du problème. Confirmation requise (« Effacer les pièces? »). Distinct de « Nouvelle modélisation » qui crée un nouveau slot. Soutient la flexibilité cognitive : repartir à zéro est parfois la meilleure stratégie.
- **Paramètres :** Bouton ⚙
...
Les actions irréversibles (Tout effacer, Reset) déclenchent un dialogue de confirmation modal avec deux boutons (Annuler / Confirmer). Pas de confirmation pour la suppression de pièce individuelle (le mécanisme « Sûr? » en 2 secondes sur le bouton Supprimer suffit).
...
- **Fond UI :** #F5F7FA
- **Surlignage zone problème :** Bleu #C5D9F0, Orange #F5D5C0, Vert #C5E8D5, Gris #D5D5D5 (Pas utile)
...
- **Tout effacer :** Repartir à zéro sans perdre l'énoncé
...
- 7 pièces sémantiques de base (Jeton, Barre, Boîte, Étiquette, Flèche, Calcul, Réponse) — 14 pièces au total avec DroiteNumérique, Tableau, Arbre, Schéma, Inconnue, DiagrammeBandes, DiagrammeLigne
...
| Feature | Notes |
|---|---|
| ~~**Action « Répartir en groupes égaux »**~~ | ✅ Livré — Action contextuelle sur les jetons libres (≥2). L'enfant choisit le nombre de groupes (2-6). L'outil crée les boîtes et y place les jetons automatiquement. Reste affiché visuellement si partage inégal. |
| ~~**Grandeurs décimales et fractions**~~ | ✅ Livré — Multipliers fractionnaires (¼×, ⅓×, ½×, ⅔×, ¾×, 1½×) dans les actions contextuelles. Subdivisions avec notation fractionnaire (showFraction). Presets de fractions courantes (2/3, 3/4, 2/5, 3/8). |
| ~~**Tableau de données**~~ | ✅ Livré — Statistique/probabilité 3e cycle. Implémentation de base (type, rendu, édition cellule). |
| ~~**Diagramme en arbre**~~ | ✅ Livré v0.2.0 — Probabilité/dénombrement 3e cycle |
| ~~**Pièce Inconnue "?"**~~ | ✅ Livré — Cercle violet avec texte éditable ("?", "x", "n"), attachable aux pièces. Mode Simplifié + Complet. Ancrage visuel de l'inconnue pour soutenir la planification (fonctions exécutives). |
| ~~**Diagramme à bandes**~~ | ✅ Livré — Bar chart statistique pour données catégorielles (PFEQ 1er-3e cycle). Axes auto-scalés, max 6 catégories, données par défaut concrètes. Mode Complet, section Structuré. |
| ~~**Diagramme à ligne brisée**~~ | ✅ Livré — Line chart pour données continues (PFEQ 2e-3e cycle). Polyline + points, axes auto-scalés, max 6 points. Mode Complet, section Structuré. |
| ~~**Deux réponses par problème**~~ | ✅ Livré — Max 2 pièces Réponse par problème. Numérotation automatique "Réponse 1" / "Réponse 2" en mode libre et à trous. |
| **Problèmes avec donnée pas utilee** | L'enfant doit sélectionner les données pertinentes — le surlignage gris rend cet exercice explicite. Ajouter 3-4 problèmes dans les catégories existantes. |

#### Accessibilité et cognition — priorisé par revue quatuor (ergo-TDC, neuropsy, pédago-PFEQ, UX)

| Priorité | Feature | Notes |
|---|---|---|
| **P1** | **Navigation clavier complète** | Seul bloquant WCAG restant. Tab entre pièces, flèches pour déplacer, Enter pour actions contextuelles. Roving tabindex sur le canvas SVG. Requis pour contacteur/clavier adapté. |
| **P2** | **Overflow toolbar tablette** | Bug silencieux : en mode Complet (15 outils), les outils débordent sans indication sur Chromebook/tablette. Étendre le patron "+Plus" au mode Complet. |
| **P3** | **Enrichir banque de problèmes** | ≥2 problèmes "construire un diagramme à bandes" (2e-3e cycle) + 3-4 problèmes avec données pas utilees. Effort faible, impact curriculaire direct. |
| **P4** | **Rappel de fatigue / nudge pause** | Nudge visuel discret à mi-parcours (timer existe déjà). Fatigue cognitive 2× plus rapide chez TDC sur tâches visuospatiales. |
| **P5** | **Vignettes dans le slot manager** | Grille visuelle avec miniatures SVG. Les enfants 6-12 ans naviguent mieux avec des images. Débloque portfolio + annotations adulte. |
| — | ~~**Mode « focus »**~~ | ✅ Livré — Toggle ActionBar (icône spotlight). Opacity 0.35 sur pièces non-sélectionnées, transition 0.4s. |
| — | ~~**Rappel flottant de l'énoncé**~~ | ✅ Livré — StatusBar affiche la question ("?") quand ProblemZone réduit. Cliquable pour ré-ouvrir. |

**Retiré — Indicateur de progression (breadcrumb) :** Consensus 3/4 agents contre. Impose une séquence rigide ; risque de "checkboxes à cocher" chez TDAH. Le rappel question dans la StatusBar remplit déjà la fonction "où en suis-je?" de façon non prescriptive.

#### Infrastructure parent / enseignant

| Feature | Notes |
|---|---|
| **Vignettes dans le slot manager** | (voir P5) Miniatures SVG stockées en base64 au save (~2-5KB). Toggle liste/grille dans "Mes travaux". |
| **Vue portfolio chronologique** | Intégrée au SlotManager (toggle liste/grille), pas modale séparée. Dépend des vignettes (P5). |
...
| **Mode parent vs mode enfant** | Verrouillage par code simple. Dépriorisé — pas critique pour l'expérience élève quotidienne. |
| **Comparaison côte à côte** | Deux modélisations en parallèle. Dépriorisé — cas d'usage avancé enseignant. |
...
- Indicateur de progression prescriptif (breadcrumb avec séquence imposée)
