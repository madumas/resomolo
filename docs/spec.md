...
| ~~**Mode « focus »**~~ | ✅ Livré — Toggle dans l'ActionBar (icône spotlight). Estompe les pièces non-sélectionnées (opacity 0.35) avec transition 0.4s. Les valeurs/labels restent lisibles pour le raisonnement relationnel. |
| ~~**Rappel flottant de l'énoncé**~~ | ✅ Livré — Quand le ProblemZone est réduit, la StatusBar affiche la question du problème (dernière phrase avec "?"). Cliquable pour ré-ouvrir le ProblemZone. |
