          ...
          {(['bleu', 'rouge', 'vert', 'jaune'] as CouleurPiece[]).map(c => (
            <button
              ...
              style={{
                ...
                border: `3px solid ${piece.couleur === c ? '#1A2433' : 'transparent'}`,
                cursor: 'pointer',
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `1px solid ${back ? '#D1D5DB' : destructive ? COLORS.destructive : active ? COLORS.primary : UI_BORDER}`,
        color: back ? '#6B7280' : destructive ? COLORS.destructive : active ? COLORS.primary : UI_TEXT_SECONDARY,
        cursor: 'pointer',
        whiteSpace: 'nowrap',
        minHeight: back ? 36 : 44,
        minWidth: back ? 36 : 44,
        display: 'flex',
        alignItems: 'center',
